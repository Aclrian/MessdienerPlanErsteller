...
import net.aclrian.mpe.utils.DateienVerwalter.CouldnotFindMedi;
...
public class Erroropener {
	...
	private static final String[] actions = { "Jetzt beheben", "Später" };
	...
	public Erroropener(Exception error) {
		...
		if (error instanceof CouldnotFindMedi) {
			...
			if (op.getValue() != null && op.getValue() == actions[0]) {
				...
				((CouldnotFindMedi) error).getMessdiener().makeXML(((CouldnotFindMedi) error).getAp());
				Utilities.logging(this.getClass(), "init",
						((CouldnotFindMedi) error).getMessdiener().toString() + " wurde behoben.");
			}
		} else {
