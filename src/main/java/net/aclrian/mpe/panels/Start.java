...
//		title.setVisible(true);
		...
		smanzeigen.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				StandartMesse sm = StandartMesseListePanel.getStandartmesse(ap);
				if (sm != null && !(sm instanceof Sonstiges)) {
					ap.getWAlleMessen().changeAP(sm);
				} else {
					Utilities.logging(this.getClass(), "init", "Die Eingabe wurde vom Benutzer abgebrochen.");
				}
			}
		});
		...
		add(pfaendern);
		pfaendern.addActionListener(e -> ap.pfarreiaendern(ap.getAda().getPfarrei()));
