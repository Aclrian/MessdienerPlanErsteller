    ...
    private static void evaluate(Statement base, Description description, int currentTry) throws Throwable {
        if (currentTry >= MAX_ITERATIONS) {
            ...
            try {
                base.evaluate();
                if (currentTry > 0) {
                    Log.getLogger().info(description.getDisplayName() + ": Success on " + (currentTry + 1) + "st/nd/rd/th Try");
                }
            } catch (Throwable t) {
                if (t instanceof RuntimeException &&
                    ...
                    Log.getLogger().warn(description.getDisplayName() + ": Error on " + (currentTry + 1) + "st/nd/rd/th Try: " + t.getMessage(), t);
