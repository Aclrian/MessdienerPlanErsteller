...
/**
 * ließt aus einer xml-Datei den Messdiener und gibt diesen zuürck
 * 
 * @author Aclrian
 *
 */
		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == 1) {
					...
					int Eintritt = Integer.parseInt(eElement.getElementsByTagName("Eintritt").item(0).getTextContent());
					...
					dienverhalten.editiereBestimmteMesse(EnumdeafaultMesse.Di_abend,
							Boolean.valueOf(eElement.getElementsByTagName("Dienstag_Abend").item(0).getTextContent()));
					dienverhalten.editiereBestimmteMesse(EnumdeafaultMesse.Sam_hochzeit,
							Boolean.valueOf(eElement.getElementsByTagName("Samstag_14:00").item(0).getTextContent()));
					dienverhalten.editiereBestimmteMesse(EnumdeafaultMesse.Sam_abend,
							Boolean.valueOf(eElement.getElementsByTagName("Samstag_Abend").item(0).getTextContent()));
					dienverhalten.editiereBestimmteMesse(EnumdeafaultMesse.Son_morgen,
							Boolean.valueOf(eElement.getElementsByTagName("Sontag_Morgen").item(0).getTextContent()));
					dienverhalten.editiereBestimmteMesse(EnumdeafaultMesse.Son_taufe,
							Boolean.valueOf(eElement.getElementsByTagName("Sontag_Taufe").item(0).getTextContent()));
					dienverhalten.editiereBestimmteMesse(EnumdeafaultMesse.Son_abend,
							Boolean.valueOf(eElement.getElementsByTagName("Sontag_Abend").item(0).getTextContent()));
					...
					try {
						...
						System.err.println(
								"Es wurde eine Alte Version von Messdiener-Dateien gefunden!" + fXmlFile.getName());
					}
